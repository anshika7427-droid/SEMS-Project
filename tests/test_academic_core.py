...
def test_subject_validation():
    ...
    with pytest.raises(ValidationError):
        SubjectCreate(name="Math", difficulty="BANANA")
...
def test_task_validation():
    ...
    with pytest.raises(ValueError, match="Task title cannot be empty or only whitespace"):
        TaskCreate(title=" ", priority="High", deadline="2026-06-30")
    ...
    from pydantic import ValidationError
    with pytest.raises(ValidationError):
        TaskCreate(title="Do HW", priority="BANANA", deadline="2026-06-30")
...
async def test_subject_service_operations(db):
    ...
    sub = await SubjectService.create_subject(db, sub_data, user_a.id)
    assert sub.name == "Maths"
    assert sub.user_id == user_a.id

    # 2. Prevent duplicate subject (case-insensitive) for the same user
    with pytest.raises(HTTPException) as exc:
        await SubjectService.create_subject(db, sub_data, user_a.id)
    assert exc.value.status_code == 400

    # 3. Allow same subject name for a DIFFERENT user
    sub_b = await SubjectService.create_subject(db, sub_data, user_b.id)
    assert sub_b.id != sub.id

    # 4. Get subject with correct ownership
    retrieved = await SubjectService.get_subject(db, sub.id, user_a.id)
    assert retrieved.id == sub.id

    # 5. Prevent getting subject owned by someone else
    with pytest.raises(HTTPException) as exc:
        await SubjectService.get_subject(db, sub.id, user_b.id)
    assert exc.value.status_code == 404

    # 6. Update subject
    update_data = SubjectUpdate(name="Advanced Maths", credits=4)
    updated = await SubjectService.update_subject(db, sub.id, update_data, user_a.id)
    assert updated.name == "Advanced Maths"
    assert updated.credits == 4

    # 7. Delete subject
    await SubjectService.delete_subject(db, sub.id, user_a.id)
    with pytest.raises(HTTPException) as exc:
        await SubjectService.get_subject(db, sub.id, user_a.id)
    assert exc.value.status_code == 404
...
async def test_task_service_operations(db):
    ...
    retrieved = await TaskService.get_task(db, task.id, user_a.id)
    assert retrieved.id == task.id
    with pytest.raises(HTTPException) as exc:
        await TaskService.get_task(db, task.id, user_b.id)
    assert exc.value.status_code == 404

    # Toggle task status
    toggled = await TaskService.toggle_task(db, task.id, user_a.id)
    assert toggled.status == "Completed"
    toggled = await TaskService.toggle_task(db, task.id, user_a.id)
    assert toggled.status == "Pending"

    # Test subject_id referencing and ownership checks in tasks
    sub_a = await SubjectService.create_subject(db, SubjectCreate(name="Sub A", difficulty="Easy"), user_a.id)
    ...
    # Delete all tasks
    deleted_count = await TaskService.delete_all_tasks(db, user_a.id)
    assert deleted_count == 2
    assert len(await TaskService.list_tasks(db, user_a.id)) == 0

# -----------------------------------
# PROGRESS & STATISTICS ACCURACY
# -----------------------------------

async def test_progress_and_statistics(db):
    user_a, user_b = await create_test_users(db)

    # Setup Subjects
    sub_math = await SubjectService.create_subject(db, SubjectCreate(name="Math", difficulty="Hard"), user_a.id)
    sub_phys = await SubjectService.create_subject(db, SubjectCreate(name="Physics", difficulty="Hard"), user_a.id)

    # Setup Tasks
    t1 = await TaskService.create_task(db, TaskCreate(title="Math HW", priority="Medium", deadline="2026-06-30"), user_a.id)
    t2 = await TaskService.create_task(db, TaskCreate(title="Physics HW", priority="Medium", deadline="2026-06-30"), user_a.id)
    # Complete t1
    ...
    m2 = await MilestoneService.create_milestone(db, MilestoneCreate(
    ...
    progress = await MilestoneService.get_progress(db, user_a.id)
    ...
    stats = await MilestoneService.get_statistics(db, user_a.id)
    ...
    empty_progress = await MilestoneService.get_progress(db, user_b.id)
    assert empty_progress.overall_progress == 0.0
    assert empty_progress.milestone_progress == 0.0

    empty_stats = await MilestoneService.get_statistics(db, user_b.id)
